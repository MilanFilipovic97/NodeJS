...
app.post("/dodajVrednostRashoda/:datum/:vrednost/:IDTroska", function (req, res) {        
    
    let sql = 'INSERT INTO lista_rashoda (Datum,Vrednost,ID_Vrste_Rashoda) values ("'+req.params.datum +'", "'+ req.params.vrednost +'","'+ req.params.IDTroska +'")';
    let query = db.query(sql, (err,result)=>{
        if(err) throw err;
        console.log(result);
        res.send('post 1 dodat');
